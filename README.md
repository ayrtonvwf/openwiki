...
> **⚠️ This is an experimental fork of [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki).**
>
> The purpose of this fork is to experiment with adding **OKF (Open Knowledge Format) output support** to OpenWiki — a code-owned frontmatter convention that makes every generated page machine-verifiable ([see the OKF output section](#okf-output)).
>
...
> | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
> | [`src/agent/okf.ts`](./src/agent/okf.ts)                                                                                                                                               | The heart of the feature — the frontmatter parse/build/stamp helpers, the OKF §9 validator and deterministic repair, root `index.md`/`log.md` generation, atomic writes, the pluggable type taxonomy, and the conformance report. |
> | [`src/agent/index.ts`](./src/agent/index.ts)                                                                                                                                           | Wires the OKF pass into the agent run, invoked only when `--okf` is enabled.                                                                                                                                                      |
> | [`src/agent/utils.ts`](./src/agent/utils.ts)                                                                                                                                           | Shared directory-walk / metadata-file-skipping traversal reused by the OKF pass.                                                                                                                                                  |
> | [`src/agent/types.ts`](./src/agent/types.ts) · [`src/agent/prompt.ts`](./src/agent/prompt.ts)                                                                                          | Run-result typing and the Phase 1/2 prompt contract for OKF-aware generation.                                                                                                                                                     |
> | [`src/commands.ts`](./src/commands.ts) · [`src/env.ts`](./src/env.ts) · [`src/startup.ts`](./src/startup.ts)                                                                           | CLI plumbing: the `--okf` / `--no-okf` / `--okf-check` flags and the `OPENWIKI_OKF` env resolution.                                                                                                                               |
> | [`src/constants.ts`](./src/constants.ts)                                                                                                                                               | `OKF_VERSION`, reserved paths (`.okf-state.json`, `log.md`), and the type taxonomy constants.                                                                                                                                     |
> | [`test/okf.test.ts`](./test/okf.test.ts) · [`test/cli-okf-check.test.ts`](./test/cli-okf-check.test.ts) · [`test/agent-okf-integration.test.ts`](./test/agent-okf-integration.test.ts) | Unit, CLI, and integration coverage, backed by conformant/non-conformant fixture bundles under [`src/agent/__fixtures__/okf`](./src/agent/__fixtures__/okf).                                                                      |
> | [`openspec/`](./openspec) · [`specs/openwiki-okf-implementation-report.md`](./specs/openwiki-okf-implementation-report.md)                                                             | The spec-first trail: each OKF capability as an OpenSpec change (proposal → design → tasks → spec) plus the full implementation report.                                                                                           |
